...
const courseCatList=[
    {
        ...
        descripton:"Discover how to mine data for insights to help inform strategic business decisions. Pursue an in"+
...
const CourseCategory = () => {
  ...
  return <div className="flex flex-col justify-center items-center gap-5">
    <h1 className="text-3xl font-semibold w-full flex justify-center py-5">Start Learning</h1>
    <div className="grid grid-cols-3 space-y-4 space-x-4  mx-56">

   
    {
        courseCatList.map((course,i)=>{
           return <a href="#" onClick={() => alert('Opening Course')} className={`${i==0?'mt-3':''}`}>
            <div className="flex items-center shadow-lg text-center border p-5 hover:text-red-600" onMouseEnter={()=>{setMoveArrow(true);setArrowId(i)}} onMouseLeave={()=>setMoveArrow(false)}>
           <div className="flex flex-col gap-4">
                <h1 className="text-xl font-semibold ">{course['title']}</h1>
                <img src={course.image} alt="Course image" className="text-black h-32" width={500}  />
                <h3 className="text-slate-500">{course.subTitle}</h3>
                <p className="text-black">{course.descripton}</p>
            </div>
            <div className={` transition-all duration-300 ${moveArrow && i==arrowId?'scale-150 translate-x-2':''} `}>
     ...
     <button className="bg-red-500 text-white py-4 px-8">Browse Course</button>
  </div>;
};
